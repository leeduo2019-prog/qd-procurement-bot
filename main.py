...
from dingtalk_notifier import send_dingtalk
...
def main():
    ...
    try:
        ...
        if results:
            ...
            crawler.save_to_file(f"procurement_notices_{ts}.json")
            crawler.save_to_csv(f"procurement_notices_{ts}.csv")

        # 无论是否有结果都发送钉钉推送
        logger.info("正在发送钉钉推送...")
        success = send_dingtalk(results)
